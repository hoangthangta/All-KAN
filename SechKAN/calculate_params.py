...
def print_parameters(model):
    ...
    for name, p in model.named_parameters():
        if p.requires_grad:
            total += p.numel()
            print(f"{name:35s} {str(tuple(p.shape)):18s} {p.numel()}")
...
# compare formula with implementation
def verify_formula(layer):

    din = layer.input_dim
    dout = layer.output_dim
    G = layer.num_grids

    actual = sum(p.numel() for p in layer.parameters() if p.requires_grad)

    feature = din * dout + dout
    basis = G + 1 + 1          # grid + scale + bias
    grid = G + 1               # grid_linear: weight + bias

    cases = [
        ("no width, no skip", False, False),
        ("width, no skip", True, False),
        ("no width, skip", False, True),
        ("width, skip", True, True),
    ]

    print("\nformula")
    print("-" * 70)
    print(f"din={din}, dout={dout}, G={G}")

    for name, use_width, use_skip in cases:

        width = 1 if use_width else 0
        base = feature if use_skip else 0

        expected = basis + width + grid + feature + base

        print(f"\n{name}")
        print(f"  sech basis      : {G} (grid) + 1 (scale) + 1 (bias)"
              + (" + 1 (width)" if use_width else ""))
        print(f"                  = {basis + width}")

        print(f"  grid linear     : {G} (weight) + 1 (bias)")
        print(f"                  = {grid}")

        print(f"  feature linear  : {din}x{dout} (weight) + {dout} (bias)")
        print(f"                  = {feature}")

        if use_skip:
            print(f"  base linear     : {din}x{dout} (weight) + {dout} (bias)")
            print(f"                  = {base}")

        print(f"  total           : {basis + width} + {grid} + {feature}"
              + (f" + {base}" if use_skip else "")
              + f" = {expected}")

        # compare with current layer configuration
        if (layer.sbf.use_width == use_width and
            layer.use_base_update == use_skip):
...
if __name__ == "__main__":
    ...
    parameter_breakdown(layer)
    verify_formula(layer)
