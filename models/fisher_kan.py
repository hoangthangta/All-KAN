...
class FisherKPPLayer(nn.Module):
    def __init__(self, D, r, dx, dt):
        super(FisherKPPLayer, self).__init__()
        self.D = D
        self.r = r
        self.dx = dx
        self.dt = dt
    
    def fisher_kpp(self, X):
        
        
        #D = 0.0001  # Diffusion coefficient
        #r = 0.1  # Growth rate
        #dx = 1.0  # Spatial step (assuming uniform grid)
        #dt = 0.01  # Time step
        X_new = torch.zeros_like(X)
        Ds = [0.2]
        for r in Ds:

            # Pad X to apply finite difference with Neumann boundary conditions
            X_pad = F.pad(X, (1, 1), mode='replicate')

            # Compute second spatial derivatives (finite difference approximation)
            d2X_dx2 = (X_pad[:, :-2] - 2 * X + X_pad[:, 2:]) / self.dx**2

            # Apply Fisher-KPP equation
            X_new += self.D * d2X_dx2 + r * X * (1 - X)
            #X_new = dX_dt * self.dt
            #X_new = dX_dt
    ...
    def forward(self, X):
        ...
        # Compute the standard deviation of the tensor
        std = X.std()
        norm_X = (X - mean) / std'''
...
class Fisher_KANLayer(nn.Module):
    def __init__(
        self,
        input_dim: int,
        output_dim: int,
        ...
        self.fisher = FisherKPPLayer(D = 0.0001, r = 0.1, dx = 0.1, dt = 0.1)
    ...
    def fisher_kpp_step(self, X, D=0.0001, r=0.1, dx=0.1, dt=0.1):
        # Parameters
        ...
        # Compute second spatial derivative (using finite difference approximation)
        X_pad = F.pad(X, (1, 1), mode='replicate')  # Pad for boundary conditions
        d2X_dx2 = (X_pad[:, :-2] - 2 * X + X_pad[:, 2:]) / dx**2
    
        # Apply Fisher-KPP equation
        dX_dt = D * d2X_dx2 + r * X * (1 - X)
        #print(dX_dt.shape)
        ...
        '''for _ in range(steps):
            X = self.fisher_kpp_step(X, D, r, dx, dt)'''
    ...
    def forward(self, x):
        ...
        fisher_output = self.fisher(x)
        
        '''for _ in range(1):
            fisher_output = self.fisher_kpp_step(fisher_output)'''
    
        fisher_output = F.linear(fisher_output, self.base_weight)
        
        return fisher_output

class Fisher_KAN(torch.nn.Module):
    ...
    def __init__(
        ...
        super(Fisher_KAN, self).__init__()
        self.grid_size = grid_size
        self.spline_order = spline_order
