...
class EfficientKAN1DProjectionLinear(torch.nn.Module):
    def __init__(
        self,
        in_features,
        out_features,
        ...
        # Project spline_order + grid_size -> 1
        self.grid_linear = nn.Linear(spline_order + grid_size, 1, bias=True)
        
        # Project in_features -> out_features
        self.feature_linear = nn.Linear(in_features, out_features, bias=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        spline_output = self.b_splines(x) # (B, in_features, grid_size + spline_order)
        spline_output = self.grid_linear(spline_output).squeeze(-1) # (B, in_features)
        spline_output = self.feature_linear(spline_output) # (B, out_features)
...
class EfficientKAN(torch.nn.Module):
    ...
    def __init__(
        ...
        grid_eps=0.02,
        grid_range=[-1, 1],
        net_type='standard' # 1d_projection
    ):
        ...
        for in_features, out_features in zip(layers_hidden, layers_hidden[1:]):
            ...
            if (net_type == 'standard'):
                ...
                self.layers.append(
                    EfficientKAN1DProjectionLinear(
                        in_features,
                        out_features,
                        grid_size=grid_size,
                        spline_order=spline_order,
